...
class DiscreteDataset(Dataset):
        ...
        def __init__(self, file_name):
            ...
            for file in os.listdir(file_name):
                d = os.path.join(file_name, file)
                if os.path.isdir(d):
                    domains.append(file)

            for d in domains:
                df = pd.read_csv(file_name + '/{}/dataset.csv'.format(d), header=0)
                frames.append(df)
            ...
            # input has to be in format batch_size x channels x state_dim x 1
            self.x_train = torch.tensor(x, dtype=torch.float32).squeeze().unsqueeze(1).unsqueeze(3)
...
def main(config):
    ...
    celeba_loader = None
    rafd_loader = None
    discrete_loader = None
    if config.env_type == 'Discrete':
        discrete_ds = DiscreteDataset(config.rafd_image_dir)
        discrete_loader = DataLoader(discrete_ds, batch_size=128, shuffle=True) # TODO: implement DiscreteLoader
    elif config.dataset in ['CelebA', 'Both']:
        celeba_loader = get_loader(config.celeba_image_dir, config.attr_path, config.selected_attrs,
                                   config.celeba_crop_size, config.image_size, config.batch_size,
                                   'CelebA', config.mode, config.num_workers)
    elif config.dataset in ['RaFD', 'Both']:
    ...
    solver = Solver(celeba_loader, rafd_loader, discrete_loader, config)
