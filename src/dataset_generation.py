...
from src.discrete_wrapper import DiscreteWrapper
from src.util import array_to_pil_format, load_baselines_model



def create_dataset(env_name, env, size, target_path, agent, agent_type="deepq", seed=None, noop_range=(0, 30),
                   epsilon=0.0, power_pill_objective=False, dataset='Discrete', domains=None, deepq_preprocessing = True,
                   ablate_agent=False):
    """
    ...
    if dataset != 'Discrete':
        ...
        wrapper = DiscreteWrapper(env_name, env)
    ...
    _generate_set(size, train_path, wrapper, agent, agent_type, dataset, domains, noop_range, epsilon)


def split_dataset(dataset_path, test_portion, domains, dataset):
    """
    ...
    for domain in domains:
        if dataset == 'Discrete':
            train_path = dataset_path + '/train/{}/dataset.csv'.format(domain)
            test_path = dataset_path + '/test/{}/dataset.csv'.format(domain)
            try:
...
def under_sample(dataset_path, dataset, domains, min_size=None):
    """
    ...
    for subset in ["train", "test"]:
        ...
        for domain in domains:
            domain_path = dataset_path + '/{}/{}/'.format(subset, domain)
            sample_names, size = _get_sample_names_and_size(domain_path)
            ...
            if dataset != 'Discrete':
                ...
                domain_path = domain_path + 'dataset.csv'
                try:
                    df = pd.read_csv(domain_path)
                    df.drop(indices_to_remove, axis=0, inplace=True)
                    df.to_csv(domain_path, index=None)
                except (EmptyDataError, FileNotFoundError):
                    continue


def create_unique_dataset(new_dataset_path, old_dataset_path, dataset):
    """
    ...
    for domain in domains:
        _save_unique_samples(train_path, old_dataset_path, domain, dataset)
        _save_unique_samples(test_path, old_dataset_path, domain, dataset)
...
def _generate_set(size, path, env_wrapper, agent, agent_type, dataset, domains, noop_range, epsilon):
    ...
    while step < size + init_steps:
            ...
            if np.random.uniform() < epsilon:
                ...
                if env_wrapper.space_invaders:
                    ...
                    if agent_type == "deepq":
                        ...
                        best_action = agent.predict(stacked_frames)[0].item()
                        output = [0] * env_wrapper.env.action_space.n
                ...
                if dataset == 'Discrete':
                    if not done:
                        preprocessed = env_wrapper.preprocess_frame(stacked_frames.squeeze())
                        collect[domains[action]].append(list(preprocessed))
                        step += 1
                else:
        ...
        if dataset == 'Discrete':
            stacked_frames, reward, done, info = env_wrapper.step(action, skip_frames=0)
        else:
            stacked_frames, observations, reward, done, info = env_wrapper.step(action, skip_frames=skip_frames)

        if done:
            stacked_frames = env_wrapper.reset()

        print(f"\rFinished {int((step + 1) / (size + init_steps) * 100)}%", end="")

    if dataset == 'Discrete':
        for c in collect.keys():
            file_name = os.path.join(path, '{}/dataset.csv'.format(c))
            df = pd.DataFrame(collect[c])
            df.to_csv(file_name, index=None)

    env_wrapper.env.close()


def _save_unique_samples(set_path, old_dataset_path, domain, dataset):
    if dataset == 'Discrete':
        path = old_dataset_path + '/{}/{}/dataset.csv'.format(set_path.split("\\")[-1], domain)
        try:
            df = pd.read_csv(path)
            unique_df = df.drop_duplicates()
            unique_path =  old_dataset_path + '_Unique/{}/{}/dataset.csv'.format(set_path.split("\\")[-1], domain)
            unique_df.to_csv(unique_path, index=None)
        except (FileNotFoundError, EmptyDataError):
            return
    else:
